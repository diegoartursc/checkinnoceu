...
/**
 * Season-themed button component with decorative elements
 */
const SeasonButton = memo(({ season, onClick }) => {
    const { buttonStyle, decor } = useMemo(() => {
        ...
        if (season.id.includes("autumn")) {
            ...
            decor = (
                <>
                    <Leaf size={20} className="absolute -top-3 -right-2 text-yellow-300 rotate-12 drop-shadow-sm animate-bounce" />
                    <Leaf size={16} className="absolute -bottom-2 -left-2 text-orange-200 -rotate-45 drop-shadow-sm" />
                </>
            );
            ...
            decor = (
                <>
                    <Snowflake size={20} className="absolute -top-3 -left-2 text-white animate-spin-slow" />
            ...
            decor = (
                <>
                    <Flower size={20} className="absolute -top-4 -right-1 text-yellow-200 animate-bounce" />
                    <Leaf size={14} className="absolute -bottom-1 left-2 text-green-300 rotate-45" />
                </>
            );
            ...
            decor = (
                <>
                    <Sun size={24} className="absolute -top-4 -left-2 text-yellow-100 animate-spin-slow" />
                </>
            );
        }
    ...
    return (
        <div className="relative group">
            ...
            <button
                onClick={onClick}
                className={`relative w-40 sm:w-48 py-3 sm:py-4 rounded-3xl shadow-2xl border-4 border-b-6 flex items-center justify-center gap-2 sm:gap-3 transition-all transform active:border-b-0 active:translate-y-2 hover:scale-105 hover:-translate-y-1 ${buttonStyle}`}
            >
