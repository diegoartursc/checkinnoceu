...
import { Cloud, Repeat2, Star, ArrowLeft } from 'lucide-react';
import { useNavigation } from '../../contexts/NavigationContext';
import { useUser } from '../../contexts/UserContext';

const TamagotchiRoom = memo(({ onOpenEveningPrayer, onOpenMonthlyLetter }) => {
  const { navigate } = useNavigation();
  const { pet, updatePet, coins, spendCoins } = useUser();
  ...
  useEffect(() => {
    ...
    if (hoursPassed > 0.1) {
      ...
      if (hungerDecay > 0 || happinessDecay > 0 || energyDecay > 0) {
        updatePet({
  ...
  const feedPet = useCallback((fruit) => {
    ...
    updatePet({
      hunger: Math.min(100, pet.hunger + fruit.hunger),
      lastUpdate: Date.now()
    });
  ...
  }, [coins, pet.hunger, spendCoins, updatePet, addFloatingText, animateAction]);
  ...
  }, [pet.energy, updatePet, addFloatingText, animateAction]);

  // Determine pet mood
  const getPetMood = useCallback(() => {
    if (!pet) return { emoji: '😐', mood: 'Carregando...', color: 'text-gray-500' };
  ...
  if (!pet) return null; // Or loading spinner
    ...
    <div className="h-full flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 h-full overflow-y-auto pb-24 pt-16 px-4 custom-scrollbar">
        ...
        <div className="flex items-center justify-between mb-8">
           <button onClick={() => navigate('checkin')} className="bg-white/50 p-2 rounded-full hover:bg-white/80 transition-colors">
        ...
        <div className="relative z-20 mb-6">
          ...
          <div className="flex justify-center items-center gap-2 px-4 mt-4">
            ...
            <div className="bg-white/60 backdrop-blur-sm rounded-full px-3 py-2 shadow-sm flex items-center gap-1.5">
              ...
              <span className="text-xs font-bold text-blue-700">{pet.energy}</span>
        ...
        <div className="mt-8 px-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            {/* Brincar - NAVIGATES TO GAMES HUB */}
            <button
            ...
            >
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="bg-pink-100 rounded-full px-2 py-0.5 flex items-center gap-1">
                  <span className="font-bold text-pink-700 text-[10px]">Ver Jogos</span>
                </div>
              </div>
            </button>
...
TamagotchiRoom.displayName = 'TamagotchiRoom';

export default TamagotchiRoom;
